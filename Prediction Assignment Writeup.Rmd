...
```{r}
library(ggplot2)
distribution_of_exercise_classes_plot <- ggplot(training_data, aes(x = classe)) +
  geom_bar() +
  theme_minimal() +
  labs(title = "Distribution of Exercise Classes", x = "Classe", y = "Count")
print(distribution_of_exercise_classes_plot)  
ggsave("distribution_of_exercise_classes.png", plot = distribution_of_exercise_classes_plot, width = 10, height = 6, dpi = 300)
```
...
# Print the plots
print(belt_plots)
ggsave("belt_plots.png", plot = belt_plots, width = 10, height = 8, dpi = 300)
```
...
These results are more easily interpreted in a bar chart.

```{r}
predictions_df <- data.frame(Classe = predicted_classes)
predictions_plot <- ggplot(predictions_df, aes(x = Classe)) + 
  geom_bar(color = "darkgrey", fill = "darkgrey") + 
  theme_minimal() + 
  labs(title = "Prediction Results", x = "Predicted Classe", y = "Count")
print(predictions_plot)
ggsave("predictions_df.png", plot = predictions_plot, width = 10, height = 6, dpi = 300)
```
